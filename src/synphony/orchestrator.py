...
from synphony.agent_runner import AgentRunner, AgentRunOutcome
...
class Orchestrator:
    ...
    def run_once(
        ...
        *,
        ...
        for issue in self.claim_dispatchable_issues(now=now):
            try:
                outcome = self.runner.run(issue, attempt_number=1, on_event=on_event)
